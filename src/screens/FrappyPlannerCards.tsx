...
const APIKEY = "AIzaSyAqP9tvqARsKjUTXNrM8-egJFwmu2YTQ1U"
class FrappyPlannerCards extends Component<IProps, Istate> {
    ...
    async componentDidMount() {
        const mode = "driving"
        const destination = "03 Kukatpally 291, 4th Phase"
        let { status } = await Location.requestPermissionsAsync()
        if (status !== "granted") {
            alert("please grant permission to access current location")
        } else {
            let location = await Location.getCurrentPositionAsync({})
            const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${location.coords}&destination=${destination}&key=${APIKEY}&mode=${mode}`
            fetch(url)
    ...
    renderRoute(coordinates: any, i: any) {
        return (
            <Polyline
                key={i}
    ...
    render() {
        return (
            <ScrollView style={styles.mainContainer}>
                ...
                >
                    {this.state.route && this.state.route.map(this.renderRoute)}
                </MapView>
